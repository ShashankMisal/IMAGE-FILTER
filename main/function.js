...
function changeSize() {
  ...
  var bgcontext = SECOND_CANVAS_ID.getContext("2d");
  bgcontext.clearRect(0, 0, FIRST_CANVAS_ID.width, FIRST_CANVAS_ID.height);
  BACKGROUND_IMG.drawTo(SECOND_CANVAS_ID);
}
...
function encrypt() {
  ...
  FOREGROUND_IMG = chop2hide(FOREGROUND_IMG);
  BACKGROUND_IMG = shift(BACKGROUND_IMG);
  ENCRYPTED_IMG = combine(FOREGROUND_IMG, BACKGROUND_IMG);
...
function decrypt() {
  var extractedImg = extract(ENCRYPTED_IMG);
...
function makeRainBow() {
  ...
  for (pixel of RAINBOW_IMG.values()) {
    X = pixel.getX();
    Y = pixel.getY();
    //      RAINBOW_IMG.setPixel(X, Y, pixel);
